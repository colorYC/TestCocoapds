...
enum {
    ...
    */
    ...
    /**
     * @brief desktop capture (screen or window) contain mouse or not.
     *  option JSON param:
     *  "enable": 0: capture without mouse; 1(default option): capture with mouse
     */
    ZmfDesktopCaptureMouse = 1<<8,

    /**
    * @brief desktop capture (screen or window) options.
    *  option JSON param:
    *  "mouse": true             capture with mouse,
    *  "disableEffects": true    indicating if desktop effects (e.g. Aero) should be disabled when the
    *                            capturer is active. Currently used only on Windows
    "  "disableCAPTUREBLT": true when using GDI BitBlt(),remove CAPTUREBLT from raster-operation  
    */
    ZmfScreenCaptureOptions = 1<<9,
    ZmfWindowCaptureOptions = 1<<10,
};
...
/**
 * @brief Post when the ZMF module video snapshot compeleted.
 * The userInfo dictionary contains the info of ZmfFile, ZmfWidth and ZmfHeight
 */
...
/**
 * @brief Start captureing desktop or window, valid on windows only.
 * @param  pcId       The ID of desktop or window.
 * @param  iFrameRate The captured frame rate in fps.
 * @param top,bottom,left,right  specifiy captured region relative to the top-left origin of the desktop or window
 * @return          0 on succeed, otherwise failed.
 */
...
/**
 * @brief Take an snapshot of video render/capture
 * @param  pcId      The render ID or capture ID
